...
- 必填字段：`name`（或 `roaster_product_name` 可兜底为主名称）、`roaster_name`、`origin_name`、`process_name`。
- 可选字段：`roaster_product_name`、`coffee_source_name`、`green_bean_merchant_name`、`green_bean_product_name`、`varietal_names`、`altitude_text`、`harvest_date_text`、`roast_date_text`、`net_weight_text`、`bean_components`、`flavor_notes`、`private_notes`。
...
- `bean_components`：拼配 / 多豆源列表，可选；每条包含产地、生产者/庄园/处理站、处理法、品种、海拔、占比/说明。
