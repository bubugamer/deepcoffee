...
```
你是 DeepCoffee 的精品咖啡知识助手。只能依据下面提供的知识库文章内容回答用户问题，用简体中文、口语化但准确地回答；如果文章里没有足够信息，就说明知识库暂无相关内容，不要编造。不要在正文里罗列引用的文章标题或来源——来源会单独展示给用户。
```
...
- 必填字段：`name`、`roaster_name`、`origin_name`、`process_name`、`varietal_names`。
- 可选字段：`roaster_product_name`、`coffee_source_name`、`green_bean_merchant_name`、`flavor_notes`、`private_notes`。
- `name`、`roaster_name`、`roaster_product_name`、`coffee_source_name`、`green_bean_merchant_name`：文本输入；如果系统已有候选实体，也可以提供下拉建议。
- `origin_name`：文本输入 + 产地候选下拉。
- `process_name`：下拉优先，允许自定义；建议选项包括水洗、日晒、蜜处理、红蜜处理、黄蜜处理、黑蜜处理、厌氧、厌氧日晒、厌氧水洗、二氧化碳浸渍、湿刨。
- `varietal_names`：多选 / 标签输入，允许自定义。
...
```
{
  ...
  "required_fields": ["name", "roaster_name", "origin_name", "process_name", "varietal_names"],
  "missing_required_fields": ["roaster_name", "process_name"],
  "fields": [
    ...
    {"key": "process_name", "label": "处理法", "required": true, "input_type": "select", "value": null, "options": ["水洗", "日晒", "蜜处理", "厌氧", "二氧化碳浸渍"]}
  ],
  "save_enabled": false,
...
- **输出处理**：解析 JSON → `BeanDraft`。`flavor` 由 `flavor_notes` 组装（有则 `source=user`，无则用默认动态模板 `酸质/甜感/醇厚/余韵/发酵感`）。字段做白名单校验；再由后端生成 `confidence`、`low_confidence_fields`、豆卡补全卡片和保存成功文案。
...
- `completed`：Coffea 已生成建议；后端已保存用户器具资料，并创建 `ai_suggestion` 隐藏冲煮记录。
...
2. 每一轮都把豆子信息、已有用户器具资料、当前会话状态、用户本轮消息发给 Coffea。
...
6. 信息充分后，后端直接保存用户器具资料和建议参数，不再二次询问用户是否确认。
7. 后端不做咖啡建议推理，但必须拒绝非法 JSON、非白名单字段、缺必填上下文和越界参数。

**system 提示词（拟）**：

```
你是 DeepCoffee 的精品咖啡冲煮顾问 Coffea。任务：围绕一支已存在的用户豆卡，完成“询问器具 → 抽取器具 JSON → 生成冲煮建议”的多轮闭环。
...
- **输出处理**：后端解析模型 JSON 并做白名单 / 必填字段 / 范围校验。`needs_input` 时只保存会话状态并把 `assistant_message` 返回前端，不写器具资料、不写建议记录。`completed` 时保存用户器具资料，解析 `recommendation` → `BrewDraft` → `complete_brew_parameters` 补算粉/水/比 → 落为 `ai_suggestion` 冲煮记录（用户不可见），豆子 `recommended_record_id` 指向它。
