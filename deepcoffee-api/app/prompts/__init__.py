...
11. 用户用文字描述一支想加入豆仓 / 建档的豆子（含烘焙商、产区、处理法、品种、烘焙日期等任一具体豆卡信息），或明确说「建豆卡 / 存一下这支豆 / 把这支豆加进豆仓」时，输出 create_or_update_bean_card，把描述解析成豆卡草稿（交用户确认后入库）。仅是询问 / 评价 / 对比某支豆（好不好喝、怎么冲、和另一支比）不要输出，走 knowledge_answer 或 direct_answer。图片里的豆卡走 read_bean_card_image，本条只针对纯文字描述。
12. 不要新增任何键。"""
...
BREW_COACH_USER_TEMPLATE = """上一轮聚焦的豆子（仅供参考；如果用户这次在指别的豆子、记录或器具，以下面的对话和清单为准，不要默认就是这一支）：
{active_bean}
