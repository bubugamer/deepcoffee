    ...
    for item in value:
        ...
        out.append(
            BeanComponent(
                ...
                green_bean_merchant_name=_clean(item.get("green_bean_merchant_name")),
                green_bean_product_name=_clean(item.get("green_bean_product_name")),
                ...
                harvest_date_text=_clean(item.get("harvest_date_text")) or _clean(item.get("harvest_date")),
...
def draft_from_bean_fields(data: dict[str, Any]) -> BeanDraft:
    ...
    components = _components(fields.get("bean_components"))
    if not components:
        legacy_component = BeanComponent(
            origin_name=_field(fields, "origin_name"),
            coffee_source_name=_field(fields, "coffee_source_name"),
            green_bean_merchant_name=_field(fields, "green_bean_merchant_name"),
            green_bean_product_name=_field(fields, "green_bean_product_name"),
            process_name=_field(fields, "process_name"),
            varietal_names=_str_list(fields.get("varietal_names")),
            altitude_text=_field(fields, "altitude_text"),
            harvest_date_text=_field(fields, "harvest_date_text"),
        )
        if any(
            [
                legacy_component.origin_name,
                legacy_component.coffee_source_name,
                legacy_component.green_bean_merchant_name,
                legacy_component.green_bean_product_name,
                legacy_component.process_name,
                legacy_component.varietal_names,
                legacy_component.altitude_text,
                legacy_component.harvest_date_text,
            ]
        ):
            components = [legacy_component]
    kwargs["bean_components"] = components
...
def summarize_draft(draft: BeanDraft) -> str:
    ...
    if draft.bean_components:
        if len(draft.bean_components) == 1:
            component = draft.bean_components[0]
            if component.origin_name:
                parts.append(f"产地 {component.origin_name}")
            if component.process_name:
                parts.append(f"处理法 {component.process_name}")
            if component.varietal_names:
                parts.append(f"品种 {'、'.join(component.varietal_names)}")
            if component.altitude_text:
                parts.append(f"海拔 {component.altitude_text}")
        else:
