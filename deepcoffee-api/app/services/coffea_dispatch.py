...
_VISUAL_REFERENCE_HINTS = ("这个", "这款", "图里", "图片", "照片", "包装", "标签", "产品", "水", "矿泉水")
_VISUAL_QUESTION_HINTS = ("？", "?", "吗", "适合", "合适", "可以", "能不能", "怎么样", "好不好", "能用")
_BREW_RECORD_HINTS = ("记录这杯", "记录这次", "记录冲煮", "冲煮记录", "帮我记录", "存这杯")
_EQUIPMENT_CAPTURE_HINTS = ("我买了", "我用了", "我用", "我的", "新买", "存", "保存", "录入", "记录")
_EQUIPMENT_NAMES = ("v60", "zp6", "c40", "法压", "爱乐压", "聪明杯", "滤杯", "磨豆机", "纸滤", "滤纸", "金属滤网")
...
def local_dispatch(
    *,
    ...
    if any(k in text for k in _BREW_RECORD_HINTS):
        return DispatchPlan(
            primary_intent="brew_record_parse",
            actions=[{"type": "brew_record_parse"}],
            source="local",
        )

    if any(k in text for k in _EQUIPMENT_CAPTURE_HINTS) and any(k in low or k in text for k in _EQUIPMENT_NAMES):
        return DispatchPlan(
            primary_intent="equipment_capture",
            actions=[{"type": "equipment_capture"}],
            source="local",
        )
...
def _looks_like_visual_knowledge_question(message: str) -> bool:
    text = message or ""
    if not text.strip():
        return False
    has_visual_reference = any(k in text for k in _VISUAL_REFERENCE_HINTS)
    has_question = any(k in text for k in _VISUAL_QUESTION_HINTS)
    return has_visual_reference and has_question


def _redirect_image_context_question(plan: DispatchPlan, message: str, attachments: Any) -> DispatchPlan:
    """带图短问句兜底：手机端常见「这个/这款……适合吗」依赖图片语义。

    调度模型若把这类问题误判成 ask_clarification，用户会看到答非所问的追问。
    这里只在「确实带图片 + 文本像知识问题 + 当前没有可看图动作」时改写为
    knowledge_answer，让执行器把原始图片交给 vision 模型回答。
    """
    if not attachments or not _looks_like_visual_knowledge_question(message):
        return plan
    handled = {
        "knowledge_answer",
        "web_verify",
    ...
    if any(isinstance(a, dict) and a.get("type") in handled for a in plan.actions):
        return plan
    if plan.primary_intent not in {"ask_clarification", "direct_answer", "out_of_scope"} and plan.actions:
        return plan
...
async def dispatch(
    *,
    ...
    plan = _redirect_image_bean_card(plan, attachments)
    return _redirect_image_context_question(plan, message, attachments)
